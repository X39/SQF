...
X39_CheckPoint_var_SpawnRoute = ["CHECKPOINT_ROUTE_%1", 9];
X39_CheckPoint_var_Queue = ["CHECKPOINT_QUEUE_%1", 7];
X39_CheckPoint_var_LeaveRoute = ["CHECKPOINT_LEAVE_%1", 11];
    ...
    while {
    ...
    } do {
        {
            if ([_vehicle, _x, _reachDistance, if !isNil "_reachSpeed" then { _reachSpeed } else { nil }] call X39_CheckPoint_fnc_DestinationReached) then {
                _remainingPositions = _remainingPositions - [_x];
            };
        } forEach _remainingPositions;
        if (time - _time > 1 && count _remainingPositions > 0) then {
            _time = time;
            [
                _vehicle,
                _remainingPositions,
